...
### Corregido
- **Auditoría seguridad: `mp-create-subscription` y `mp-check-subscription` revisadas.** Ambas validan JWT con `getUser` y derivan `company_id` server-side vía `get_user_company_id(user.id)` — el cliente nunca provee `company_id`, por lo que no existe vector cross-tenant. No requieren `user_belongs_to_company`. Limitación funcional documentada: operan sobre la empresa default del usuario; soportar billing en empresas secundarias requeriría aceptar `company_id` explícito + membership check (fuera del scope de seguridad).
