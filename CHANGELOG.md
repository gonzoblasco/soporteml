...
### Agregado
- **EL-7 · Fase 1 — Alertas de SLA por email (Resend).** Nueva edge function `sla-breach-notifier` programada vía pg_cron cada 5 min: barre cada empresa con `sla_escalation_enabled = true` y emails configurados, busca preguntas pendientes que pasaron `sla_target_minutes` sin responder y aún no fueron notificadas, y manda un único resumen HTML con comprador, texto y minutos de espera (de a 50 por empresa, agrupando para evitar spam). Dedupe vía nueva columna `questions.sla_alert_sent_at` (con índice parcial sobre pending+sin responder+sin alerta); un trigger limpia la marca si la pregunta se reabre (`answered_at` vuelve a NULL). Nueva columna `company_settings.sla_alert_emails text[]` y campo nuevo en **Settings → Operación → SLA y escalation** para listar destinatarios separados por coma (validación de formato en el cliente, deshabilitado si el toggle está apagado). Función protegida por `X-Cron-Secret` o body `{"source":"cron"}`, registrada en `config.toml` con `verify_jwt = false`.
